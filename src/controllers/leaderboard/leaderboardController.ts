...
import { LeaderboardService, LeaderboardQuery } from "../../services/leaderboard";
...
export const getLeaderboardData = async (
  ...
  try {
    ...
    Logger.info(`Fetching leaderboard data: ${scope} ${location} ${metric} ${timeframe}`);

    const query: LeaderboardQuery = {
      scope: scope as "global" | "country" | "city" | "continent",
      location: scope === "global" ? undefined : location as string,
      metric: metric as "commits" | "pullRequests" | "issues" | "stars" | "overall",
...
export const populateLeaderboardForLocation = async (
  ...
  try {
    ...
    if (!location && scope !== "global") {
      ApiResponseUtil.badRequest(res, "Location is required for non-global scope");
      return;
    }
    ...
    if (result.success) {
      ApiResponseUtil.success(res, result, "Leaderboard populated successfully");
    } else {
      ApiResponseUtil.internalError(res, result.message || "Failed to populate leaderboard");
    }
  } catch (error) {
...
export const syncDevelopersData = async (
  ...
  try {
    ...
    ApiResponseUtil.success(res, null, "Developers data sync completed successfully");
  } catch (error) {
...
export const getDeveloperProfile = async (
  ...
  try {
    ...
    const developer = await leaderboardService.getDeveloperByHandle(githubHandle);
