...
export class DeveloperScoringEngine {
  ...
  calculateScore(metrics: DeveloperMetrics): number {
    ...
    score += normalizedMetrics.languageDiversity * this.weights.languageDiversity;
  ...
  private normalizeMetrics(metrics: DeveloperMetrics): NormalizedMetrics {
    return {
      ...
      activity: metrics.contributionsLast6Months ? 
        this.logNormalize(metrics.contributionsLast6Months, 1, 2000) : 0
    };
  }
  ...
  static calculateLanguageDiversity(languages: Array<{name: string, count: number}>): number {
  ...
  static calculateRepoQuality(repos: Array<{
